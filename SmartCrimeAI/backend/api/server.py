...
def _zone_dict(zone: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        "gnn_hotspot_prob": _to_native(getattr(zone, "gnn_hotspot_prob", 0.0)),
    }
...
<head>
  ...
  <style>
    ...
    * {
    ...
    .zone-cell.patrol-route-step {
      border: 1.5px dashed var(--accent-blue) !important;
      box-shadow: inset 0 0 10px rgba(59, 130, 246, 0.2);
    }
    
    .zone-cell.patrol-route-primary {
...
<body>
  ...
  <div class="dashboard-container">
    ...
    <!-- Left column: Map & Hotspot Intel -->
    <div style="display: flex; flex-direction: column; gap: 1.5rem; align-self: start; width: 100%;">
      
      <!-- Map Visualizer -->
      <div class="glass-card map-container" style="margin: 0; width: 100%;">
        ...
        <!-- Patrol Route Legend -->
        <div id="patrol-legend" class="patrol-legend-card">
          <span style="font-size: 1.1rem;">👮‍♂️</span>
          <span id="patrol-legend-text" style="color: var(--text-secondary);">Hover or click a police officer dot to trace their active patrol route.</span>
        </div>
      </div>

      <!-- AI Hotspot & Lighting Intel Card -->
      <div class="glass-card" style="width: 100%;">
        <div class="card-title">AI Hotspot & Lighting Intelligence</div>
        <div style="font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.75rem;">
          Real-time monitoring of active GNN/RF hotspots and local street lighting:
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 0.85rem; text-align: left;">
            <thead>
              <tr style="border-bottom: 1px solid var(--border); color: var(--text-secondary);">
                <th style="padding: 0.4rem; font-weight: 600;">Zone ID</th>
                <th style="padding: 0.4rem; font-weight: 600;">Zone Type</th>
                <th style="padding: 0.4rem; font-weight: 600; text-align: center;">RF Risk</th>
                <th style="padding: 0.4rem; font-weight: 600; text-align: center;">GNN Hotspot %</th>
                <th style="padding: 0.4rem; font-weight: 600; text-align: center;">Lighting Level</th>
                <th style="padding: 0.4rem; font-weight: 600; text-align: right;">Status</th>
              </tr>
            </thead>
            <tbody id="hotspot-intel-table-body">
    ...
    <div class="sidebar">
      ...
      <!-- AI Patrol Benchmarks Card -->
      <div class="glass-card">
        <div class="card-title">AI Patrol Benchmarks</div>
        <div style="display: flex; flex-direction: column; gap: 0.75rem;">
          <div style="font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.25rem;">
            Side-by-side performance of active patrol strategies:
          </div>
          
          <table style="width: 100%; border-collapse: collapse; font-size: 0.85rem; text-align: left;">
            <thead>
              <tr style="border-bottom: 1px solid var(--border); color: var(--text-secondary);">
                <th style="padding: 0.4rem 0.2rem; font-weight: 600;">Strategy</th>
                <th style="padding: 0.4rem 0.2rem; font-weight: 600; text-align: center;">Crimes</th>
                <th style="padding: 0.4rem 0.2rem; font-weight: 600; text-align: center;">Caught</th>
                <th style="padding: 0.4rem 0.2rem; font-weight: 600; text-align: right;">Catch %</th>
              </tr>
            </thead>
            <tbody>
              <tr style="border-bottom: 1px solid rgba(255, 255, 255, 0.05);">
                <td style="padding: 0.6rem 0.2rem; font-weight: 600; color: var(--accent-blue);">Greedy</td>
                <td id="bench-greedy-crimes" style="text-align: center; color: var(--text-secondary);">0</td>
                <td id="bench-greedy-caught" style="text-align: center; color: var(--accent-blue);">0</td>
                <td id="bench-greedy-rate" style="text-align: right; font-weight: bold; color: var(--text-primary);">0.0%</td>
              </tr>
              <tr style="border-bottom: 1px solid rgba(255, 255, 255, 0.05);">
                <td style="padding: 0.6rem 0.2rem; font-weight: 600; color: var(--accent-purple);">Central RL</td>
                <td id="bench-rl-crimes" style="text-align: center; color: var(--text-secondary);">0</td>
                <td id="bench-rl-caught" style="text-align: center; color: var(--accent-purple);">0</td>
                <td id="bench-rl-rate" style="text-align: right; font-weight: bold; color: var(--text-primary);">0.0%</td>
              </tr>
              <tr>
                <td style="padding: 0.6rem 0.2rem; font-weight: 600; color: var(--accent-orange);">MARL Co-op</td>
                <td id="bench-marl-crimes" style="text-align: center; color: var(--text-secondary);">0</td>
                <td id="bench-marl-caught" style="text-align: center; color: var(--accent-orange);">0</td>
                <td id="bench-marl-rate" style="text-align: right; font-weight: bold; color: var(--text-primary);">0.0%</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  <script>
    ...
    let currentPatrolRoutes = {};
    let activeHighlightedOfficer = null;
    let isHighlightLocked = false;

    function highlightRoute(officerId) {
      if (isHighlightLocked && activeHighlightedOfficer !== officerId) return;
      
      // Clear any existing highlight first
      clearRouteHighlightInternal();
      
      activeHighlightedOfficer = officerId;
      const route = currentPatrolRoutes[officerId];
      const legendText = document.getElementById('patrol-legend-text');
      const legend = document.getElementById('patrol-legend');
      
      if (!route || route.length === 0) {
        legendText.innerHTML = `Officer <b>${officerId}</b> does not have an active patrol route.`;
        legend.style.background = 'rgba(239, 68, 68, 0.05)';
        legend.style.borderColor = 'rgba(239, 68, 68, 0.15)';
        return;
      }
      
      legend.style.background = 'rgba(59, 130, 246, 0.08)';
      legend.style.borderColor = 'rgba(59, 130, 246, 0.3)';
      legendText.innerHTML = `Active Route for <b>${officerId}</b>: ` + route.map((z, idx) => {
        return `<span style="font-weight: 600; color: #60a5fa;">${z}</span>`;
      }).join(' ➔ ') + (isHighlightLocked ? ' <span style="font-size:0.75rem;color:var(--accent-orange);margin-left:8px;">[LOCKED]</span>' : '');
      
      route.forEach((zoneId, index) => {
        const cell = document.getElementById(`cell-${zoneId}`);
        if (cell) {
          if (index === 0) {
            cell.classList.add('patrol-route-primary');
          } else {
            cell.classList.add('patrol-route-step');
          }
          
          // Add small step number badge if not already present
          let badge = cell.querySelector('.patrol-step-badge');
          if (!badge) {
            badge = document.createElement('div');
            badge.className = 'patrol-step-badge';
    ...
    function clearRouteHighlight() {
      ...
      legend.style.borderColor = '';
      legendText.innerHTML = 'Hover or click a police officer dot to trace their active patrol route.';
      activeHighlightedOfficer = null;
    }
    
    function clearRouteHighlightInternal() {
      // Remove classes and badges from all cells
      for (let r = 0; r < 6; r++) {
        for (let c = 0; c < 6; c++) {
          const id = `${rows[r]}${c}`;
          const cell = document.getElementById(`cell-${id}`);
          if (cell) {
            cell.classList.remove('patrol-route-primary', 'patrol-route-step');
            const badge = cell.querySelector('.patrol-step-badge');
            if (badge) {
              badge.remove();
            }
          }
    ...
    function toggleRouteLock(officerId, event) {
      if (event) event.stopPropagation(); // prevent grid cell click triggers
      
      if (activeHighlightedOfficer === officerId && isHighlightLocked) {
        // Unlock and clear
        isHighlightLocked = false;
        clearRouteHighlight();
      } else {
        // Lock to this officer
        isHighlightLocked = false; // reset lock temporarily to allow change
        highlightRoute(officerId);
        isHighlightLocked = true;
        // Update legend to show locked state
        const legendText = document.getElementById('patrol-legend-text');
        legendText.innerHTML = `Active Route for <b>${officerId}</b>: ` + currentPatrolRoutes[officerId].map((z) => {
          return `<span style="font-weight: 600; color: #60a5fa;">${z}</span>`;
        }).join(' ➔ ') + ' <span style="font-size:0.75rem;color:var(--accent-orange);margin-left:8px;">[LOCKED]</span>';
      }
    }
    
    // Clear lock when clicking anywhere else on the page
    document.addEventListener('click', (e) => {
      const isOfficerDot = e.target.classList.contains('agent-dot') && e.target.classList.contains('police');
      if (!isOfficerDot && isHighlightLocked) {
        isHighlightLocked = false;
        clearRouteHighlight();
      }
    });
    ...
    function updateUIWithState(state) {
      // Update global patrol routes mapping
      currentPatrolRoutes = state.patrol_routes || {};
      
      // If an officer is currently highlighted, dynamically refresh their route overlay
      if (activeHighlightedOfficer) {
        highlightRoute(activeHighlightedOfficer);
      }
      ...
      state.agents.forEach(agent => {
        ...
        if (container) {
          ...
          if (agent.type === 'police') {
            icon = '👮';
            dot.title = `Officer: ${agent.id} (${agent.state})\nClick to lock/unlock route.`;
            dot.style.cursor = 'pointer';
            
            // Attach mouse and click event listeners for route tracing
            dot.addEventListener('mouseenter', () => highlightRoute(agent.id));
            dot.addEventListener('mouseleave', () => clearRouteHighlight());
            dot.addEventListener('click', (e) => toggleRouteLock(agent.id, e));
      ...
      // --- 5. Update AI Hotspot & Lighting Intel Table ---
      const tableBody = document.getElementById('hotspot-intel-table-body');
      if (tableBody) {
        tableBody.innerHTML = '';
        const hotspots = state.zones.filter(z => z.is_hotspot);
        ...
        if (hotspots.length > 0) {
          hotspots.forEach(z => {
            ...
            // Format risk & GNN prob
            const rfPct = (z.risk_score * 100).toFixed(1);
            const gnnPct = ((z.gnn_hotspot_prob || 0.0) * 100).toFixed(1);
            const lightPct = (z.lighting * 100).toFixed(0);
            
            // Type badge color
            let badgeStyle = 'background: rgba(255,255,255,0.05); color: var(--text-secondary);';
            if (z.zone_type === 'commercial') badgeStyle = 'background: rgba(59, 130, 246, 0.15); color: var(--accent-blue);';
            else if (z.zone_type === 'residential') badgeStyle = 'background: rgba(16, 185, 129, 0.15); color: var(--accent-green);';
            else if (z.zone_type === 'park') badgeStyle = 'background: rgba(245, 158, 11, 0.15); color: var(--accent-orange);';

            // Light warning status
            let lightWarning = '🟢 Normal';
            if (z.lighting < 0.45) lightWarning = '🔴 Blackout/Dim';
            else if (z.lighting < 0.75) lightWarning = '🟡 Low Light';

            tr.innerHTML = `
              <td style="padding: 0.6rem 0.4rem; font-weight: bold; color: var(--text-primary);">${z.id}</td>
              <td style="padding: 0.6rem 0.4rem;">
                <span style="font-size: 0.75rem; padding: 0.15rem 0.4rem; border-radius: 0.25rem; ${badgeStyle}">
                  ${z.zone_type.toUpperCase()}
                </span>
              </td>
              <td style="padding: 0.6rem 0.4rem; text-align: center; font-weight: bold; color: #f87171;">${rfPct}%</td>
              <td style="padding: 0.6rem 0.4rem; text-align: center; font-weight: bold; color: #c084fc;">${gnnPct}%</td>
              <td style="padding: 0.6rem 0.4rem; text-align: center; font-weight: bold; color: var(--accent-orange);">${lightPct}%</td>
              <td style="padding: 0.6rem 0.4rem; text-align: right; font-weight: 600;">${lightWarning}</td>
            `;
          ...
          tableBody.innerHTML = `
            <tr>
              <td colspan="6" style="padding: 2rem; text-align: center; color: var(--text-secondary); font-style: italic;">
                🟢 No active hotspots detected. Standard patrol parameters.
              </td>
            </tr>
          `;
        }
    ...
    function updateMetrics(metrics) {
      ...
      // Update AI Patrol Benchmarks Card dynamically
      if (metrics.mode_counters) {
        const greedy = metrics.mode_counters.greedy || { crimes: 0, caught: 0 };
        const rl = metrics.mode_counters.ai || { crimes: 0, caught: 0 };
        const marl = metrics.mode_counters.marl || { crimes: 0, caught: 0 };

        document.getElementById('bench-greedy-crimes').innerText = greedy.crimes;
        document.getElementById('bench-greedy-caught').innerText = greedy.caught;
        const gRate = greedy.crimes > 0 ? (greedy.caught / greedy.crimes * 100) : 0.0;
        document.getElementById('bench-greedy-rate').innerText = `${gRate.toFixed(1)}%`;

        document.getElementById('bench-rl-crimes').innerText = rl.crimes;
        document.getElementById('bench-rl-caught').innerText = rl.caught;
        const rlRate = rl.crimes > 0 ? (rl.caught / rl.crimes * 100) : 0.0;
        document.getElementById('bench-rl-rate').innerText = `${rlRate.toFixed(1)}%`;

        document.getElementById('bench-marl-crimes').innerText = marl.crimes;
        document.getElementById('bench-marl-caught').innerText = marl.caught;
        const marlRate = marl.crimes > 0 ? (marl.caught / marl.crimes * 100) : 0.0;
        document.getElementById('bench-marl-rate').innerText = `${marlRate.toFixed(1)}%`;
      }
    }
